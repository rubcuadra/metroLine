...
public:
    ...
    GraphicDoubleNode(const T & _data, sf::Color _color = sf::Color::Yellow, int _radius = 50);
    void configure(const T & _data, sf::Color _color = sf::Color::Yellow, int _radius = 50);
    void configure(sf::Color _color = sf::Color::Yellow, int _radius = 50);
...
template <class T>
GraphicDoubleNode<T>::GraphicDoubleNode(const T & _data, sf::Color _color, int _radius)
{
    configure(_data, _color, _radius);
}
template <class T>
void GraphicDoubleNode<T>::configure(const T & _data, sf::Color _color, int _radius)
{
    // Store the data for the TreeNode
    this->setData(_data);
    // Configure the visual representation
    configure(_color, _radius);
}
template <class T>
void GraphicDoubleNode<T>::configure(sf::Color _color, int _radius)
{
